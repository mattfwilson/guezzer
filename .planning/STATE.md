...
- [ui] **LiveGizz** tracking screen (4 tweaks): center the pre-opener orb; rename "Tap the opener"→"Search for the opener" + make it tappable to open Search (`CenterNode.tsx`, `config.ts:333`); remove the blank `SuggestionStrip` bar above the tabs (keep 56px reserved for SHOW-02, drop border/bg when empty); move End Show from the header into the FAB (`FabMenu.tsx` + `ShowView.tsx:332-342`) — `.planning/todos/pending/2026-07-17-livegizz-tracking-screen-opener-orb-search-center-strip-ends.md`. Owner idea 2026-07-17.

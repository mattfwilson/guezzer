 ...
 * over the live (still-growing) trail + the LIVE dex snapshot on every render, so
 * it re-lights on every `logSong` as a pure consequence of `session.entries`
 * changing. The `nearMiss` one-away detector picks the single closest target and
...
import { dexSnapshot, getBingoContext } from "../games/bingoContext.ts";
...
export function BingoPeekStrip({ card, entries }: BingoPeekStripProps) {
  const copy = config.copy.games.bingo;
  // The memoized marking context (built once) + the LIVE dex snapshot (the
  // growing dex, NOT a frozen snapshot — deriveLiveBoard's contract, D-04).
  ...
  // trail + live dex; recomputed only when they (or the card) change. Marks are
  // never stored (D-23).
  const derived = useMemo(() => {
    if (!ctxResult || !dex) return null;
    const caught = dexSnapshot(dex);
    const board = deriveLiveBoard(card, entries, ctxResult.ctx, caught);
    const miss = nearMiss(board.marked, card, ctxResult.ctx, caught);
    return { board, miss };
  }, [ctxResult, dex, card, entries]);

# Requirements Archive: v2.0 Multi-User Foundation
...
- [x] **AUTH-01**: A friend can sign in with a pre-made email/password (owner-minted) and reach the app as their distinct identity — no self-service registration.
- [x] **AUTH-02**: The app boots offline from a restored, unexpired session (`getSession()` synchronously from localStorage) — startup is NEVER gated on a live network auth check, so a dead-signal venue still opens the app. `onAuthStateChange` reconciles when connectivity returns. *(Highest-risk item — must not regress the v1 offline boot.)*
- [x] **AUTH-03**: The signed-in user can see who they are logged in as (seeded `display_name`) surfaced in the app chrome (header/menu chip).
- [x] **AUTH-04**: The user can sign out; on a shared/borrowed device this is the identity hand-off mechanism.
- [x] **AUTH-05**: On first login, the existing single-user Dexie data is namespaced to the logged-in user id exactly once — a borrowed or shared phone never cross-contaminates two friends' dexes.
- [x] **AUTH-06**: The app ships the "Gizz With Friends" rebrand — wordmark, document title, and PWA manifest copy — signalling the multi-user identity. Display labels/chrome only: tab routes, file paths, and persisted Dexie/storage keys stay unchanged (same discipline as the v1 tab rebrand). *(Header title already renamed in commit 30f86cc; remaining surfaces here.)*
- [x] **AUTH-07**: Each identity has a deterministic auto color/avatar derived from the user id (same idiom as tuning/rarity colors) — no uploads, no storage — legible for presence dots and friend rows in the dark. Feeds PROG + PRES.
- [x] **AUTH-08**: A stale access token reconnects gracefully — a calm "reconnecting…" affordance rather than a jarring "you're logged out," so the offline→online transition and deferred writes land without a perceived logout. *(Blueprint open-item handled as UX, not an architecture change.)*
...
*Last updated: 2026-07-24 — AUTH-01..08 checkboxes + traceability reconciled to Complete at v2.0 close (Phase 18 shipped + device-UAT-passed 2026-07-22; audit-flagged bookkeeping lag)*

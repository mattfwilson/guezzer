...
Resolution: **Confirmed closed set — no new variants anywhere in 2010–2026 among KGLW (`artist_id === 1`) rows.** Full corpus: `"Set"` (9,562 rows / 696 shows), `"One Set"` (210 rows / 42 shows), `"Live Session"` (86 rows / 19 shows). No "Soundcheck", festival, or other label ever appears in KGLW rows. `config.settypeAllowlist = ["Set", "One Set"]` is confirmed final, not provisional — D-16's exclusion of `"Live Session"` (19 shows total, not just the 2 in the 2013 sample) is the only exclusion needed.

**Addendum (plan 01-04 Task 1):** the census above intentionally scopes settype counting to `artist_id === 1` (01-03-SUMMARY.md decisions), by design — it never counted side-project rows' settype. Because `rawSetlistRowLocked` (D-11) validates every raw row structurally *before* the artist_id filter runs, its settype vocabulary must additionally cover values that appear only on side-project rows in the same committed multi-artist tables. A direct scan of every `data/raw/setlists-*.json` file (all artist_ids) found two more: `"DJ Set"` (86 rows, e.g. `show_id 1747399797`, 2025-05-25, artist_id 36 "King Gizzard & The Lizard Wizard DJ's") and `"Act"` (1 row, `show_id 1685039426`, 2020-03-01, artist_id 9 "Professor of the Occult"). `settypeLocked` includes both for drift-detection purposes only; `config.settypeAllowlist` is unaffected since it is applied after the artist_id filter has already dropped every side-project row.

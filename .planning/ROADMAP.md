...
- [ ] **Phase 22: Surface Motion & the Chrome Mechanism** - Animate the one shared sheet primitive without losing an a11y guarantee; debut chrome-hide on GizzVerse with its escape and a11y invariants proven (all 10 plans executed 2026-08-06; verification `gaps_found` — device UAT unrun, CHROME-05 overclaimed, 2 code-review blockers open)
...
**Success Criteria** (what must be TRUE):
  ...
  1. Every **`<Sheet>`-backed, prop-driven** sheet animates smoothly up on open and down on close with a scrim cross-fade, and under `prefers-reduced-motion` appears and disappears without motion (SHEET-01). *Amended 2026-08-10 from "Every bottom sheet": five hand-rolled sheets (incl. `SearchSheet`, the most-used surface at a show) and six enter-only openings are documented seams — see REQUIREMENTS.md SHEET-01 and `todos/pending/2026-08-10-migrate-hand-rolled-bottom-sheets-to-the-sheet-primitive.md`.*
  ...
  3. In GizzVerse, one tap hides the top bar and bottom tabs for more constellation viewing area and a control that stays visible in the same place restores them — and hiding or showing fires exactly **one** resize callback, asserted by test, which cannot move the layout or snap the camera (CHROME-01, CHROME-05). *Amended 2026-08-10 from "never reheating the simulation or degrading battery": the shipped code reheats on every `size.height` change, but pinned `fx`/`fy` and the first-settle camera gate make that reheat **inert, not absent** — see REQUIREMENTS.md CHROME-05 and `todos/pending/2026-08-10-constellation-reheats-on-pure-box-change.md`.*
...
- [x] 22-09-PLAN.md — Capability meta tags (own commit) + 22-HUMAN-UAT.md device script
...
| 22. Surface Motion & the Chrome Mechanism | v2.1 | 10/10 | Gaps Found | 2026-08-06 |

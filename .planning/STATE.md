...
- [ui] **LiveGizz** tracking (3 edits): move the FAB down to 16px above the tab bar (equal to its 16px right inset — **flag: conflicts with the reserved SuggestionStrip mid-show**); make comet-trail nodes solid-filled + 50% smaller (`TRAIL_NODE_*` 24/40→12/20) + connect them with a line as a chronological path; change prediction-orb info from tapping the (i) to a **long-press** on the orb (**flag: a11y regression — long-press has no keyboard/AT equivalent**) — `.planning/todos/pending/2026-07-17-livegizz-fab-spacing-solid-connected-comet-trail-long-press.md`. Owner idea 2026-07-17.

...
- [ui] **GizzDex** Albums: always render the full album shelf (grayed via §B4 dimming, `AlbumGrid.tsx:77-81`) even at zero catches instead of the EmptyState, and drop the Mark-attended CTA from Albums so it's Shows-only (`DexView.tsx:117-139`) — `.planning/todos/pending/2026-07-17-gizzdex-albums-always-show-full-grayed-grid-mark-attended-on.md`. Owner idea 2026-07-17; mostly subtractive, clean up orphaned `emptyDex`/`EmptyState`.

...
**Success Criteria** (what must be TRUE):
  ...
  5. Long-retired songs sink as designed (the era-prior floor is reachable), and the user can reset cross-night rotation state before a new run/weekend so prior-run songs are no longer suppressed when the no-repeat window no longer applies (PRED-02, PRED-03).**Plans**: 5 plans across 4 waves

**Wave 1**

  - [x] 11-01-PLAN.md — Wave 0: rewrite the masking era-prior test at production scale (RED gate for PRED-02)
  - [x] 11-02-PLAN.md — Wave 1: live-path core — guardLatestRows (LIVE-01), lenient schema + detectNovelKeys + PollResult (LIVE-03), mixed-artist regression (LIVE-02)
...
**Plans**: 4 plans, 1 wave (independent fixes, parallel-eligible)
  ...
  - [x] 13-03-PLAN.md — UX-03: rewrite pure-core resolvePlaceholders to interval-count-match anchoring (correct-or-suppress, never off-by-N) + regression fixtures
  - [x] 13-04-PLAN.md — UX-04: first-settle ref gates constellation zoomToFit (camera survives resize) + off-screen focus pan re-center + FOCUS_OFFSCREEN_MARGIN_PX
...
- [x] 14-01-PLAN.md — Foundations: BingoCard contract + zod schema, string-seeded PRNG (xmur3/mulberry32), config.bingo scaffold

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 14-02-PLAN.md — buildBingoContext (shipped-artifact lookups) + detectWins (4×4 geometry) + shared fixtures

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 14-03-PLAN.md — deriveMarks consume-once fold + live==replay==catch-up / tie-break tests (load-bearing)
- [x] 14-04-PLAN.md — deal seeded card generator + same-seed / never-blank tests
...
- [x] 14-06-PLAN.md — D-20 process: roster candidates → human sign-off checkpoint → lock constants (GATE 2 cleared)
...
**Wave 2** *(blocked on Wave 1)*
  ...
  - [x] 15-02-PLAN.md — app persistence: Dexie `version(5)` + `bingoCards` table + `BingoCardRow` + `saveDraftCard`/`lockCard` freeze + reshuffle guard (SC-1) + snapshot/import threading + `SCHEMA_VERSION` 3 + Phase-15 copy (BINGO-07)

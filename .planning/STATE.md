...
Last activity: 2026-08-12 -- Completed quick task 260811-uxw: FOV 2026 schedule artifact re-transcribed from the final official poster (not yet redeployed)
...
| 260811-pxg | Gave the product name ONE owner per package and removed the drift mechanism. `APP_NAME` declared above the `config` literal in **both** `packages/app/src/config.ts` and `packages/core/src/config.ts` (two owners, not one, because `app/config.ts:13` records **D-39** — leaf module, type-only imports — so it must not take a runtime import from core); 15 app copy strings + 3 component JSX literals (`AppMenu` sheet heading, `AppShell` header, `SignInScreen` h1) routed through `config.copy.appName`, and core's two user-facing import-rejection strings in `data-safety/merge.ts` through `config.appName`. `storageNotProtected` reworded rather than token-swapped (a mechanical swap yields the possessive "Gizz With Friends's"). **NAV-02 held: `config.DB_NAME` still `"guezzer"`, `GuezzerDB` unrenamed** — renaming orphans every saved dex. Guard added to `test/rebrand.test.ts` scanning **both** src trees, asserting the brand literal lives in exactly the two config files and "Guezzer" only in `app/db/db.ts` + `core/config.ts` (userAgent); observed FAILING against 5 injected probes before being trusted. **The core half was missed on the first pass and found by grepping the built bundle, not the source** — `merge.ts` contains a literal NUL byte, so plain `grep -rn` reports "Binary file matches" and silently omits its lines; use `grep -a`. 140 files / 1231 tests, tsc exit 0 | 2026-08-11 | cf4934e | [260811-pxg-route-user-facing-guezzer-app-name-strin](./quick/260811-pxg-route-user-facing-guezzer-app-name-strin/) |
| 260811-uxw | Re-transcribed the **final official FOV 2026 poster** (`FOV2026_SCHEDULE_FINAL.jpg`, as published on fieldofvision.meadowcreekco.com/KBYG) into `data/schedule/fov-2026.json` — six deltas vs the 7.26 transcription: three mystery `?????????` sets named (**Stu Mackenzie** Fri Castle, which ALSO moved 1:00PM→**12:30PM** start; **Cavs Gong Bath** Sat Castle; **Bullant** Sun Castle), Fri Castle 1:30AM late-night swapped **Moktar → Nikki Nair**, the **Buena Vista High School Red Hots Jazz Band moved days** (Sat 12:00–12:40 → Sun 11:00–11:40), and the `source` string re-cited. **Every surviving event id kept byte-identical** — `fri-castle-1300` still carries its 1300 slug despite now starting 12:30, because `sanitizeEventIds` drops unknown ids SILENTLY, so an id rename is a silent deletion of every friend's saved pick for that set; pinned by a permanence guard asserting `fri-castle-1230` does not exist. Only sanctioned churn: `sat-castle-1200` deleted, `sun-castle-1100` added (that set genuinely moved days, so its Saturday picks are correctly lost). Timeland rows, `days`, `venues` untouched — the poster does not cover the Timeland stage. Stale `Moktar` prose examples of the >1440 past-midnight convention refreshed in `schedule.ts` + the test. TDD: RED run failed for exactly the 3 intended assertions before GREEN. 140 files / 1234 tests green, core+app tsc exit 0. **Outstanding: the Timeland panel was NOT re-verified — KBYG prose says the Drag Me Outside hikes run "11:30am and 2:30pm" Fri/Sat, but the artifact has the second hike at 2:00PM (`fri/sat-timeland-1400`, startMin 840). Check against the Timeland graphic before the show.** Data-only + bundled at build time ⇒ friends need a redeploy to see it | 2026-08-12 | 5ec356b | [260811-uxw-update-fov-2026-schedule-artifact-from-o](./quick/260811-uxw-update-fov-2026-schedule-artifact-from-o/) |
